...
(function() {
  var Collapse, CollapseView, animationEndEvents, cv;
  ...
  CollapseView = Backbone.View.extend({
    ...
    nextItem: function() {
      ...
      if ((this.currentItem + 1) >= this.totalItems) {
        ...
        next = $(this.items[this.currentItem + 1]);
    ...
    toggle: function() {
      var $this, i, item, items, j, len, number;
      ...
      for (i = j = 0, len = items.length; j < len; i = ++j) {
        ...
        $this = $(item);
        if (!this.allOpen) {
          this.$el.queue('toggle', this.open.bind(this, item, i));
        } else {
          this.$el.queue('toggle', this.close.bind(this, item, i));
        }
      }
      this.$el.queue('toggle', (function(_this) {
        return function(next) {
          _this.allOpen = _this.allOpen ? false : true;
          return next();
        };
      })(this));
  ...
  Collapse = (function() {
    ...
    Collapse.prototype.$ = function(selector) {
      return this.$el.find(selector);
    };
    ...
    Collapse.prototype.init = function() {
      ...
      this.items.each(function(index) {
        var $this;
        $this = $(this);
        this.dataset.number = index;
        if (index > 0) {
          return $this.hide();
        }
      });
      $('.collapse-all').click(this.toggleAll.bind(this));
      $('.collapse-prev').click(this.previous.bind(this));
      return $('.collapse-next').click(this.next.bind(this));
    };

    Collapse.prototype.toggleAll = function() {
      this.items.each((function(_this) {
        return function(index, item) {
          var $item;
          $item = $(item);
          if (!_this.allOpen) {
            return $item.css('position', 'relative');
          } else {
    ...
    Collapse.prototype.show = function(number) {
      var current, next;
      current = this.items[this.currentItem];
      next = this.items[number];
      this.hideAll(next);
      $(current).removeClass('slideInRight slideInLeft').addClass('animated fadeOut');
      return $(next).show().removeClass('section-small slideOutLeft slideOutRight').addClass('animated fadeIn');
    };

    Collapse.prototype.previous = function(e) {
      var current, next;
      current = this.items[this.currentItem];
      if ((this.currentItem - 1) < 0) {
        next = this.items[this.totalItems - 1];
      ...
      $(current).removeClass('slideInRight slideInLeft').addClass('animated slideOutRight');
      return $(next).show().removeClass('slideOutLeft slideOutRight').addClass('animated slideInLeft');
    };

    Collapse.prototype.next = function(e) {
      var current, next;
      current = this.items[this.currentItem];
      if ((this.currentItem + 1) >= this.totalItems) {
        next = this.items[0];
      ...
      $(current).removeClass('slideInRight slideInLeft').addClass('animated slideOutLeft');
      return $(next).show().removeClass('slideOutLeft slideOutRight').addClass('animated slideInRight');
    };

    Collapse.prototype.animationEnd = function(element, callback) {
      return element.one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', callback.bind(this));
    };
